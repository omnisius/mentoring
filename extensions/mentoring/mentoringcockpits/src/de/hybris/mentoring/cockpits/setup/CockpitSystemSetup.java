 ...
 *
 */
package de.hybris.mentoring.cockpits.setup;
...
import de.hybris.mentoring.cockpits.constants.MentoringCockpitsConstants;
 ...
 */
@SystemSetup(extension = MentoringCockpitsConstants.EXTENSIONNAME)
public class CockpitSystemSetup extends AbstractSystemSetup
{
	...
	public void createEssentialData(final SystemSetupContext context)
	{
		...
		{
			if (MediaManager.getInstance().getMediaFolderByQualifier(MentoringCockpitsConstants.JASPER_REPORTS_MEDIA_FOLDER).size() < 1)
			{
				MediaManager.getInstance().createMediaFolder(MentoringCockpitsConstants.JASPER_REPORTS_MEDIA_FOLDER,
						MentoringCockpitsConstants.JASPER_REPORTS_MEDIA_FOLDER);
			}
			try
			{
				String prefix = null;
				...
				if (prefix != null){
					importImpexFile(context, "/mentoringcockpits/reportcockpit/import/" + prefix + "_jasperreports.impex");
				}
			}
